...
function initGroupeAnimations() {
  initSectionTimeline(
    ...
    (section, timeline) => {
      ...
      if (title) timeline.add(toAnimation(title, ANIMATION_TYPES.APPEAR_Z, { duration: 1.08 }));

      const fadeLeftEls = section.querySelectorAll('[data-anim="2"]');
      if (fadeLeftEls.length) {
        timeline.add(toAnimation(fadeLeftEls, ANIMATION_TYPES.FADE_LEFT, { stagger: 0.156, duration: 1.08 }), '-=0.45');
      }

      const images = section.querySelectorAll('[data-anim="3"]');
      if (images.length) {
        timeline.add(toAnimation(images, ANIMATION_TYPES.FADE_UP, { stagger: 0.144, duration: 1.08 }), '-=0.45');
      }
    },
  );
}
